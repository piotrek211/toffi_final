...
namespace argos {
	class AutoMoDeFsmBuilder {
		...
		private:
			...
			AutoMoDeBehaviour* GetAutoMoDeBehaviour(UInt8 unBehaviourIdentifier);
			AutoMoDeCondition* GetAutoMoDeCondition(UInt8 unConditionIdentifier);
			std::vector<std::string> GetPossibleParameters(UInt8 param_type);
