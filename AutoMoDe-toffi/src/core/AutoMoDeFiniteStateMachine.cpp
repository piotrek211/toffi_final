...
namespace argos {
	...
	void AutoMoDeFiniteStateMachine::MaintainHistory() {
		...
		switch (m_unType){
		case 0:
			sHistoryPath << m_strHistoryFolder << "./fsm_history_" <<  (static_cast<EpuckDAO*>(m_pcRobotDAO))->GetRobotIdentifier() << ".txt";
			break;
		case 1:
			sHistoryPath << m_strHistoryFolder << "./fsm_history_" <<  (static_cast<SmartObjectDAO*>(m_pcRobotDAO))->GetRobotIdentifier() << ".txt";
			break;
		}
	//HistoryPath << m_strHistoryFolder << "./fsm_history_" <<  m_pcRobotDAO->GetRobotIdentifier() << ".txt";
	...
	void AutoMoDeFiniteStateMachine::SetRobotDAO(void* pc_robot_DAO, UInt32 type) {
		m_unType = type;
